...
from .rscnn_utils import pytorch_utils as pt_utils
from .rscnn_utils.pointnet2_modules import PointnetSAModule, PointnetFPModule, PointnetSAModuleMSG
import numpy as np
...
class RSCNN_MSN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        c_in = cfg.network.input_channels
        self.SA_modules.append(  # 0
            PointnetSAModuleMSG(
                ...
                relation_prior=cfg.network.relation_prior
            )
        )
        c_out_0 = 64 * 3
        ...
        self.SA_modules.append(  # 4   global pooling
            PointnetSAModule(
                nsample=16,
                mlp=[c_out_3, 128], use_xyz=cfg.network.use_xyz
            )
        )
        global_out = 128
        ...
        self.SA_modules.append(  # 5   global pooling
            PointnetSAModule(
                nsample=64,
                mlp=[c_out_2, 128], use_xyz=cfg.network.use_xyz
            )
        )
        global_out2 = 128
        ...
        self.FP_modules.append(
            PointnetFPModule(mlp=[256 + cfg.network.input_channels, 128, 128])
        )
        ...
        self.FC_layer = nn.Sequential(
            ...
            pt_utils.Conv1d(128, cfg.num_classes, activation=None)
        )
